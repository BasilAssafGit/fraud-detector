...
async function prepareFileContent(file) {
  ...
  if (isText) return { mode:"text", text:await file.text() };
  if (isPdf || isImage) {
...
async function extractFromDocument(fc) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1800, system:EXTRACTION_SYSTEM, messages }),
  });
  if (!res.ok) {
...
export default function App() {
  ...
  const handleFile = useCallback((f) => {
    if (!f) return;
    const ok = /\.(pdf|docx?|png|jpe?g|txt)$/i.test(f.name) ||
      ["application/pdf","image/png","image/jpeg","text/plain",
       "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
       "application/msword"].includes(f.type);
    if (!ok) { setErrMsg(`Unsupported: ${f.name}. Use PDF, DOCX, PNG, JPEG, or TXT.`); return; }
      ...
      <div style={{padding:"24px 32px",maxWidth:1200,margin:"0 auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 360px",gap:24,alignItems:"start"}}>
          ...
          <div>
            ...
            >
              <input ref={inputRef} type="file" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg,.txt" onChange={e=>handleFile(e.target.files[0])}/>
              {!file ? (
                <>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
                  <div style={{fontSize:30}}>{file.type==="application/pdf"?"📄":file.type.startsWith("image/")?"🖼️":"📝"}</div>
